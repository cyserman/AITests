// Auto-load case data on first run
(function () {
    const CASE_DATA_KEY = 'CASE_CRAFT_STATE_V3';
    const LOADED_FLAG = 'CASE_DATA_LOADED';

    // Only load once
    if (localStorage.getItem(LOADED_FLAG)) {
        console.log('✅ Case data already loaded');
        return;
    }
    ...
    localStorage.setItem(CASE_DATA_KEY, JSON.stringify(caseData));
    localStorage.setItem(LOADED_FLAG, 'true');

    console.log('✅ Your case data has been loaded!');
    console.log('📊 Loaded 5 case events:');
    console.log('  - PFA order filed (PKT-001)');
    console.log('  - Camper incident (EVT-0002)');
    console.log('  - July 4th exchange incident (CL-003)');
    console.log('  - Counsel withdrawal (EVT-0004)');
    console.log('  - Christmas holiday denial (EVT-0005)');
    console.log('\n🔄 Refreshing to load your data...');
